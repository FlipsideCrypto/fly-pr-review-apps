...
if [ "$EVENT_TYPE" = "closed" ]; then
  ...
  # todo -- drop the corresponding database
  exit 0
fi
...
if [ -n "$INPUT_POSTGRES" ]; then
  ...
  output=$(flyctl postgres attach "$INPUT_POSTGRES" --app "$app" --database-name "$app"-db --force)
  if [[ "$output" == *"is now attached"* ]]; then
    should_seed=true
  fi
fi
...
if [ "$should_seed" = true ]; then
  ...
  flyctl ssh console --command "yarn prisma db seed" --app "$app"
fi
